...
const handleStartCommand = async ctx => {
	...
	const text = await handleGenerateText(
		ctx,
		`${instructions.translator}First, greet and introduce yourself in the user's language, use one thematic emoji, and ask them to select a language to translate to. Break the response into paragraphs.`,
